...
def compute_kpi_inventory_turnover(
    ...
    """
    # Monthly average inventory value by warehouse
    # Note: F.date_format() inside withColumn must NOT use .alias() — the column name
    # is set by withColumn's first argument. Using .alias() inside withColumn raises
    # AnalysisException on some Spark/Delta versions.
    df_inv_monthly = (
        df_inventory
    ...
    # Monthly COGS by warehouse_key — fact_sales has warehouse_key from dim join
    df_cogs_monthly = (
        df_sales
