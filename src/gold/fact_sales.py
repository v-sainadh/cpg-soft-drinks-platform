...
def compute_fact_sales(
    ...
    - return_amount      = 0 (returns tracked separately; join to reduce in Gold if needed)

    Dimension keys resolved:
    - date_key    from dim_date on transaction_date
    - product_key from ref_products on sku_id
    - customer_key from customers (current record) on retailer_id
    ...
    # Current products only (SCD Type 1, so all rows are current)
    ...
    # Current customers only (SCD Type 2 — filter is_current)
    df_cust_keys = (
        df_customers
    ...
    df = (
        ...
        .select(
            ...
            F.col("transaction_date"),   # keep for partition
        )
    )
